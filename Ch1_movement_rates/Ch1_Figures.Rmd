...
plot_grid(A, B, C, D, labels = c("A)", "B)", "C)", "D)"), hjust = 0, vjust = 1, label_size = 10)
...
plot_grid(logfit_plot, social_group_curves, movrates_used, nrow = 3, rel_heights = c(2,2,1), labels = c("A)", "B)", "C)"), hjust = 0, vjust = 1, label_size = 10)
...
```{r Figure3, fig.width=3, fig.height=3}
...
cp.dens +
  ...
  geom_hline(yintercept = 0.00005, linetype = "dashed", color = "black") +
  labs(y = "") -> cp.tail
...
cbind(c("Distance", "CP", "PP", "NP"), rbind(dist_range,
                                             signif(pextRemes(evd.fit.cp, dist_range, lower.tail = FALSE), 5),
                                             signif(pextRemes(evd.fit.pp, dist_range, lower.tail = FALSE), 5),
                                             signif(pextRemes(evd.fit.np, dist_range, lower.tail = FALSE), 5)))
