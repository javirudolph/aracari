...
## Figure Dispersal Kernels

```{r}
cp_lines <- purrr::map(seq(1:1000), function(y)
  stat_function(fun = dweibull, args = list(scale = weib.boot.cp$est.scale[y], shape = weib.boot.cp$est.shape[y]), color = "lightgray"))

pp_lines <- purrr::map(seq(1:1000), function(y)
  stat_function(fun = dweibull, args = list(scale = weib.boot.pp$est.scale[y], shape = weib.boot.pp$est.shape[y]), color = "lightgray"))

np_lines <- purrr::map(seq(1:1000), function(y)
  stat_function(fun = dweibull, args = list(scale = weib.boot.np$est.scale[y], shape = weib.boot.np$est.shape[y]), color = "lightgray"))
