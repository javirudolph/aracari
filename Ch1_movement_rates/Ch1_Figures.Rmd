...
points_plot <- function(seed.data, s.size = 1000, xylims=1250, lalpha = 0.3){
  seed.data %>% 
    drop_na(s.id) %>% 
    mutate(ov500 = ifelse(disp <500, "non", "ldd")) %>% 
    sample_n(s.size) %>% 
      ggplot(., aes(x = xloc, y = yloc, color = ov500)) +
...
A <- points_plot(seed.data = cp.df)
B <- points_plot(seed.data = pp.df)
C <- points_plot(seed.data = np.df)
