...
load("Ch1_movement_rates/sims_backup/weib_cpr.RData")
load("Ch1_movement_rates/sims_backup/weib_ppr.RData")
load("Ch1_movement_rates/sims_backup/weib_npr.RData")
...
cpr.summ.df %>%
  dplyr::select(av.disp, dsprsn, model) %>% 
  bind_rows(., ppr.summ.df %>%
              dplyr::select(av.disp, dsprsn, model)) %>%
  bind_rows(., npr.summ.df %>%
              dplyr::select(av.disp, dsprsn,  model) %>%
              mutate(model = "npr")) %>% 
  mutate(model = factor(model, levels = c("cpr", "ppr",
                                          "npr"))) -> disp_r_df
```


# Figure x - average dispersal and dispersion per run  
...
disp_r_df %>%
  ...
  labs(title = "Dispersion", y = "Dispersion in meters", x = "Model") +
  theme_bw() +
  theme(legend.position = "none") -> pr_dispersion
...
disp_r_df %>%
  ...
  labs(title = "Average dispersal", y = "Dispersal in meters", x = "Model") +
  theme_bw() +
  theme(legend.position = "none") -> pr_dispersal

plot_grid(pr_dispersal, pr_dispersion)
```
...
weib.boot.cpr %>%
  dplyr::select(., est.shape, est.scale, boot, model) %>%
  bind_rows(., weib.boot.ppr %>%
              dplyr::select(., est.shape, est.scale, boot, model)) %>%
  bind_rows(., weib.boot.npr %>%
              mutate(model = "npr") %>%
              dplyr::select(., est.shape, est.scale, boot, model)) -> joint_r_weib
...
## Variation 1 - violin plot with 100 out of 1000 of the parameter fits
...
joint_r_weib %>%
  ...
  geom_jitter(position = position_jitter(0.1)) +
  labs(title = "Scale") +
  theme_bw() -> weib_r_scale

plot_grid(weib_r_shape, weib_r_scale)
