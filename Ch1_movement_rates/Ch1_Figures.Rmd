...
seedplots2d <- plot_grid(A, B, C, ncol = 3, labels = "A")
#seedplots2d
...
plot_grid(seedplots2d, disp2_plot, nrow = 2)
...
### EVD generalized pareto results
...
nint <- 70 # number of intervals. how many thresholds to try
r <- c(0, 700) # range over which to try the thresholds
nsamps <- 500 # random sample form the distances
...
```{r}
a <- read.table("Ch1_movement_rates/dispdistancestest.txt")
a


threshrange.plot(a$V1, type = "GP", nint = 100, r = c(0,700))
c <- mrlplot(a$V1)
mrlplot(a$V1)
...
model <- lm(d$`Mean Excess`~1)
confint(model, level = 0.9)


b <- decluster(a$V1, threshold = 170)

threshrange.plot(b)

fit <- fevd(b, threshold = 170, type = "GP")
plot(fit)
fit$results

1-pevd(1000,threshold = 170, scale= fit$results$par[1], shape = fit$results$par[2], type = "GP")

# OR

dist_range <- c(1000, 5000, 10000)
pextRemes(fit, dist_range, lower.tail = FALSE)
