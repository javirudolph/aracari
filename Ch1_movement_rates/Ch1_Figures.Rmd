...
Too many points, since we have 500k seeds, take 2%

```{r}

points_plot <- function(seed.data, summ.data, s.size = 25, xylims=1250, lalpha = 0.5){
  seed.data %>% 
    drop_na(s.id) %>% 
  ...
  plot_grid(seedlocplot, dispersionplot, ncol=2, rel_widths = c(2,1))
  # 
}

A <- points_plot(seed.data = cp.df, summ.data = cp.summ.df)

A

# B <- points_plot(seed.data = pp.df, summ.data = pp.summ.df)
# C <- points_plot(seed.data = np.df, summ.data = np.summ.df)
# 
