...
prms.df %>% 
  ...
  mutate(best_col = ifelse(deltaBIC == 0, "#ffaa00", "black"))-> nice_prms
```
...
model_comp_IC %>% 
  dplyr::select(data, dist, BIC) %>% 
  group_by(data) %>% 
  mutate(delta_within = signif(BIC - min(BIC), digits = 2)) %>% 
  ungroup() %>% 
  mutate(delta_across = signif(BIC - min(BIC), digits = 2)) %>% 
  dplyr::select(., -data)-> data_gt

gt_tbl <- gt(data_gt, rowname_col = "dist")

gt_tbl %>% 
  tab_header(
    title = md("**BIC comparisons across levels**")
  ) %>% 
  ...
  cols_label(
    delta_within = html("&Delta; BIC<sub>within</sub>"),
    delta_across = html("&Delta; BIC<sub>across</sub>")
  )
...
plot_params <- function(prm_data){
  ...
  prm_data %>% 
    ...
    theme(legend.position = "none",
          axis.text.x = element_text(angle = 45, color = prm_data$best_col))
}
...
prms.fams.df %>% 
  ...
  mutate(deltaBIC = signif(BIC - min(BIC), 3),
         deltaAIC = signif(AIC - min(AIC), 3),
         deltaAICc = signif(AICc - min(AICc), 3)) -> popfam.IC
...
saveRDS(all_indiv_prms, file = "appendixA/popindprms.RDS")
saveRDS(all_fam_prms, file = "appendixA/popfamprms.RDS")
