...
# Here, we are taking our tracking data and removing mpm (meters moved per minute) that are equal to zero, since these won't allow for distributio fittings. We also create a new variable for Bird ID, and modify net squared displacement. 
df <- ptpl %>%
  dplyr::filter(mpm != 0) %>%
  ungroup() %>%
