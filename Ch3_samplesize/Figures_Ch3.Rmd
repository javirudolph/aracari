...
# Figure 1 - the mixture scenarios considered
...
truth_df %>%
  ...
  geom_point(data = tru_tail[1:50,], aes(x = values, y = y), color = "grey47", alpha = 0.5, size = 1) +
  geom_vline(xintercept = thresh_tests, color = dens_cols[4], linetype = "dashed", size = 0.3) +
  labs(y = "Frequency in Population", x = "Distance") +
  #lims(x = c(0, 150)) +
  theme_bw(base_size = 5) -> truth_hist
# truth_hist
...
top_row <- plot_grid(means_sd_plot, densities_plot, truth_density, nrow = 1, rel_widths = c(1,2,2), labels = "AUTO", label_size = 6)

plot_grid(top_row, truth_hist, nrow = 2, labels = c("", "D"), label_size = 6)
...
nonpar_glm %>% 
  mutate(Original = theta_hat - theta,
         # Corrected = log_theta_stars - theta,
         Corrected = theta_bar - theta,) %>% 
  dplyr::select(thresh_tests, samp_n_tests, Original, Corrected) %>% 
  pivot_longer(cols = c(Original, Corrected), names_to = "type") %>% 
  filter(., !is.infinite(value)) %>% 
  # filter(., thresh_tests < 500) %>% 
  mutate(thresholdsep = factor(ifelse(thresh_tests<500, "lower dist", "extremes"), levels = c("lower dist", "extremes"))) %>% 
  mutate(samp_n_tests = factor(samp_n_tests),
         thresh_tests = factor(thresh_tests)) %>% 
  mutate(labelsamp = paste("n =", samp_n_tests),
         labelsamp = factor(labelsamp, levels = c("n = 80", "n = 200", "n = 500", "n = 800", "n = 1000", "n = 1600"))) %>%
  mutate(onelinelabel = paste0(labelsamp,"-",thresholdsep)) %>% 
  ...
  scale_fill_manual(values = pal_sampsize, name = "Sample Size") +
  labs(x = "Distance level", y = "Estimates of theta", subtitle = " ") +
  theme_bw() 
...
GP_mles %>% 
  ...
  scale_fill_manual(values = pal_sampsize, name = "Sample Size") +
  scale_color_manual(values = pal_sampsize, name = "Sample Size") +
  labs(x = "Long-distance level", y = "log(GP_theta/theta)", title = "Generalized Pareto") +
  theme_bw() +
  theme(legend.position = "none") -> a
