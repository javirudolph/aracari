...
pp.boot.fits %>%
  ...
  pivot_wider(names_from = param, values_from = estimate) -> pp.best
...
```{r FigureX, fig.height=12, fig.width=12}
library(MetBrewer)

colsx <- met.brewer("Gauguin", n = 7)
aval <- 0.3
...
cp.grid <- plot_grid(NULL, cp.ln, cp.wei, ncol = 3)
...
pp.grid <- plot_grid(pp.gam, pp.ln, pp.wei, nrow = 1)
...
np.grid <- plot_grid(np.gam, np.ln, np.wei, nrow = 1)

plot_grid(cp.grid, pp.grid, np.grid, nrow = 3)
```
