...
```{r Figure1, fig.width=12, fig.height=5.5}
ptpl %>%
  drop_na(mpm) %>%
  filter(mpm != 0) %>%
  group_by(Bird_ID) %>%
...
# To know the number of speed data points for each individual
# nonzerompm %>% group_by(Bird_ID) %>% count() %>% arrange(n)

# To know the number of speed data points per interval category
# nonzerompm %>% group_by(T_minutes) %>% count() %>% mutate(percent = n/nrow(nonzerompm))
...
nonzerompm %>%
  ...
  theme(legend.position = "bottom") -> np_ints
  
  #theme(legend.position = "none") -> np_ints

# This is the information on the bursts and number of relocations per individual 

# ptpl %>% group_by(burst) %>% add_tally(name = "burstn") %>% 
#   dplyr::select(Bird_ID, burstn) %>% 
#   filter(., burstn >= 2) %>% 
#   distinct() %>% #summary()
#   #group_by(., Bird_ID) %>% count() %>% arrange(n)
#   ggplot(., aes(x = Bird_ID, y = burstn)) + 
#   geom_violin() +
...
ptpl %>% 
  ...
  theme(axis.text = element_blank()) -> np_angles

plot_grid(np_ints, np_angles, nrow = 2, rel_heights = c(1, 0.7), labels = c("A)", "B)"), label_size = 10)
 
ggsave("Ch2_distributions/Figures/Figure1.png")
```
...
# These are the plot and legend for the number of observations and intervals when we consider the data at the family level. 
...
```{r Figure2, fig.width=12, fig.height=4.5}
...
```{r Figure1old, fig.height=9, fig.width=12, eval = FALSE}
...
```{r Figure3, fig.height=9, fig.width=9}
...
```{r Figure4, fig.width=12, fig.height=5}
...
ggsave2(filename = "Ch2_distributions/Figures/Figure5.png")
...
```{r FigureNA, eval=FALSE, fig.width=8, fig.height=6}
p1_angles <- ptpl %>% 
  drop_na(rel.angle) %>% 
  mutate(d.angle = ifelse(rel.angle > 0, rel.angle * 180/pi,
                          rel.angle * 180/pi + 360)) %>% 
...
# Some info on the angles df

# angles_df %>% 
#   drop_na(rel.angle) %>% group_by(individual) %>% count() %>% arrange(n)
```
