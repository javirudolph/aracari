# 20211207 Data analysis

# Libraries -----------------------------------------------
set.seed(20211207)

library(aracari)
library(dplyr)
library(ggplot2)
library(tidyr)
library(purrr)
library(fitdistrplus)

# Data ----------------------------------------------------
# The script for this data is in the data-raw folder
#load("data/ptpl.rda")

# ptpl %>%
#   filter(., mpm != 0) -> ptpl

load("Ch2_distributions/Orig_data_KH/tidy_data.RData")


# Focus on R2n
# R2n is the net squared displacement between the first relocation and every subsequent relocation.
# We don't have long-enough time series for each of these bursts, and they are spaced out in different time intervals
# So calculate an animal's velocity using the sqrt(R2n) over the time.


ptpl %>%
  dplyr::select(id, sgroup, date, burst, R2n, dt) %>%
  mutate(sqR2n = sqrt(lead(R2n))) %>%
  group_by(burst) %>%
  mutate(ct0 = ifelse(is.na(dt) == T, 0, dt),
         ct = cumsum(ct0)/60) %>%
  ungroup() %>%
  drop_na(dt) %>%
  mutate(vel.R2n = sqR2n/ct) %>%
  filter(vel.R2n != 0) %>%
  group_by(id) %>%
  add_tally() %>%
  filter(n >= 30) -> r2n_ptpl

View(r2n_ptpl)



ptpl %>%
  mutate(T_minutes = dt/60,
         Bird_ID = id,
         mpm = dist/T_minutes,
         R2n = lead(R2n)) %>%
  filter(mpm != 0) %>%
  group_by(Bird_ID) %>%
  add_tally() %>%
  filter(n >= 30) -> ptpl


dist.used <- c("gamma", "weibull", "lnorm")
param <- c("shape", "rate", "shape", "scale", "meanlog", "sdlog")
dist <- c("gamma", "gamma", "weibull", "weibull", "lnorm", "lnorm")
kpars <- c(2, 2, 2, 2, 2, 2)


# Complete pooling fits -------------------------------------

## Function to save the fit information and parameters -----

build_fits_df <- function(x){

  nm <- deparse(substitute(x))

  x %>%
    map(., `[`, c("estimate", "sd", "loglik", "n", "bic")) %>%
    map(as_tibble) %>%
    bind_rows()
}

# Regular fits -----------------------------------------

## CP fit ------------------------------------------------

cp.fit <- lapply(dist.used, function(x){fitdist(r2n_ptpl$vel.R2n, distr = x)})

reg_fits_info <- build_fits_df(cp.fit) %>%
  mutate(param = param,
         dist = dist,
         kpars = kpars,
         dBIC = bic - min(bic),
         ID = "CP",
         model = "CP")

## NP fit ---------------------------------------------

ids <- unique(r2n_ptpl$id)

for(j in 1:length(ids)){

  indv.df <- r2n_ptpl %>%
    filter(., id == ids[j])

  fit <- lapply(dist.used, function(x){fitdist(indv.df$vel.R2n, distr = x)})

  out <- build_fits_df(fit) %>%
    mutate(param = param,
           dist = dist,
           kpars = kpars,
           dBIC = bic - min(bic),
           ID = paste0("B",ids[j]),
           model = "NP")

  reg_fits_info <- rbind.data.frame(reg_fits_info, out)

}

## PP fit -----------------------------------------------

g.id <- paste0("G", 1:7)

for(j in 1:length(g.id)){

  fam.df <- r2n_ptpl %>%
    filter(., sgroup == g.id[j])

  fit <- lapply(dist.used, function(x){fitdist(fam.df$vel.R2n, distr = x)})

  out <- build_fits_df(fit) %>%
    mutate(param = param,
           dist = dist,
           kpars = kpars,
           dBIC = bic - min(bic),
           ID = toupper(g.id[j]),
           model = "PP")

  reg_fits_info <- rbind.data.frame(reg_fits_info, out)

}

#################################################################

###################################################################


# Bootstrapping --------------------------------------------------
## CP bootstrapping -----------------------------------------

nboots <- 10
nsamples <- length(r2n_ptpl$vel.R2n)

cp.boot.fits <- NULL

for(i in 1:nboots){

  boot.df <- sample(r2n_ptpl$vel.R2n, size = nsamples, replace = TRUE)

  fit <- lapply(dist.used, function(x){fitdist(boot.df, distr = x)})

  out <- build_fits_df(fit) %>%
    mutate(param = param,
           dist = dist,
           kpars = kpars,
           dBIC = bic - min(bic),
           boot = paste0("boot_", i))

  cp.boot.fits <- rbind.data.frame(cp.boot.fits, out)
}


# No pooling fits ----------------------------------

ids <- unique(r2n_ptpl$id)

np.boot.fits <- NULL


for(i in 1:nboots){

  boot.df <- r2n_ptpl %>%
    ungroup() %>%
    dplyr::select(.,id, vel.R2n) %>%
    group_by(id) %>%
    nest() %>%
    ungroup() %>%
    mutate(n = map_int(data, nrow)) %>%
    mutate(samp = map2(data, n, sample_n, replace = TRUE)) %>%
    dplyr::select(-data) %>%
    unnest(samp)

  for(j in 1:length(ids)){

    indv.df <- boot.df %>%
      filter(., id == ids[j])

    fit <- lapply(dist.used, function(x){fitdist(indv.df$vel.R2n, distr = x)})

    out <- build_fits_df(fit) %>%
      mutate(param = param,
             dist = dist,
             kpars = kpars,
             dBIC = bic - min(bic),
             individual = paste0("B", ids[j]),
             boot = paste0("boot_", i))

    np.boot.fits <- rbind.data.frame(np.boot.fits, out)

  }

}



# Partial pooling fits ----------------------------------

g.id <- paste0("G", 1:7)

pp.boot.fits <- NULL


for(i in 1:nboots){

  boot.df <- r2n_ptpl %>%
    ungroup() %>%
    dplyr::select(., sgroup, vel.R2n) %>%
    group_by(sgroup) %>%
    nest() %>%
    ungroup() %>%
    mutate(n = map_int(data, nrow)) %>%
    mutate(samp = map2(data, n, sample_n, replace = TRUE)) %>%
    dplyr::select(-data) %>%
    unnest(samp)

  for(j in 1:length(g.id)){

    fg.df <- boot.df %>%
      filter(., sgroup == g.id[j])

    fit <- lapply(dist.used, function(x){fitdist(fg.df$vel.R2n, distr = x)})

    out <- build_fits_df(fit) %>%
      mutate(param = param,
             dist = dist,
             kpars = kpars,
             dBIC = bic - min(bic),
             group = toupper(g.id[j]),
             boot = paste0("boot_", i))

    pp.boot.fits <- rbind.data.frame(pp.boot.fits, out)

  }

}



# save.image(file = "Ch2_distributions/fits_df.RData")
save(nboots, dist, dist.used, ids, g.id, kpars, param, reg_fits_info,
     cp.boot.fits, pp.boot.fits, np.boot.fits,
     file = "Ch2_distributions/Ch2_R2n_fits.RData")
