...
#ggsave("testfig.tiff", dpi = 300, width = 3, units = "in")
```
...
## Figure 4. 
Seed dispersal kernels produced by the three models considered with Weibull distribution overlaid
...
null_dispersal %>% 
  ...
  geom_histogram(aes(y = ..density..), binwidth = 50) +
  stat_function(fun = dweibull, n = 100, args = list(shape = null_weibull$estimate[1], scale = null_weibull$estimate[2]),
                linetype = "solid", size = 1) +
  theme_bw() +
  labs(y = "Density", "Seed dispersal (m)") +
  lims(y = c(0, 0.004), x = c(0, 1500))
```
...
all_disp_data %>% 
  ggplot(., aes(y = mean_dispersal, group = model)) +
  #facet_wrap(~model, ncol = 3) +
  geom_boxplot() +
  theme_bw()  +
  labs(y = "Seed dispersal (m)") -> b
