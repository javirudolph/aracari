...
dispersal_data %>% 
  ggplot() +
  geom_density(aes(x = dispersal, group = id), color = "grey") +
  geom_density(aes(x = dispersal), color = "red") +
...
dispersal_data %>% 
  ...
  stat_density(aes(linetype = "Simulated data"), geom = "line", position = "identity") +
...
plot_grid(C, CC, CCC, nrow = 3, labels = "AUTO")
...
```{r}
aligned_plots(null_mrl, null_threshplot, orig_thres, title = "Null model")
aligned_plots(indiv_mrl, indiv_threshplot, orig_thres, title = "Individual model")
aligned_plots(fam_mrl, fam_threshplot, orig_thres, title = "Family model")
```
