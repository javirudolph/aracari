...
Where the shape parameter $a$ determines the tail of the distribution, and $b$ is the scale parameter. It is worth mentioning that the standard parameterization of the Weibull in 'R' is different from the parameterization used in Morales and Carlo [-@Morales_effects_2006], where the shape parameter $a = \nu$ remains the same, but the scale parameter is defined as $\kappa = b^{-\nu}$, giving a density distribution function of $W(r)= \kappa\nu r^{\nu-1} exp[-{\kappa r}^\nu]$. Here, we report the scale parameter following the alternative parameterization, but the original values returned after fitting the Weibull distribution using R can be found in the Supplementary Table 1. Since the shape parameter remains the same, we can note that for $a=\nu=1$ the distribution shows a tail with exponential decay, with values of $\nu <1$ the tail shows fast-decay, and when $\nu>1$ we can see a fat-tailed distribution (Fig 2.) We visually assessed the fit of the distributions via qqplots and used the Kolmogorov-Smirnov statistic to assess goodness of fit (Supplementary information).

Fig 2. Capation: Variation in Weibull distributions determined by the value of the shape parameter. The scale parameter is defined as $b=1$, and the shape parameters are defined as follows: solid line $a=\nu=1$ shows an exponential decay tail, dashed line $a=\nu=0.2$ shows a thin tail, and $a=\nu=2$ describes a fat tail with the dotted line.
